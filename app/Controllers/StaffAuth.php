    ...
    public function attempt()
    {
        ...
        if (! $this->validate($rules)) {
            return redirect()->back()
                   ->with('val_errors', $this->validator->getErrors())
                   ->with('toast',['type'=>'danger','msg'=>'Revisá los campos.'])
                   ->withInput();
        }
        ...
        if (! $row || !hash_equals((string)$row['password'],$pass)) {
            return redirect()->back()
                ->with('toast',['type'=>'danger','msg'=>'Correo o contraseña incorrectos.'])
                ->withInput();
        }

        $isAdmin = ((int)$row['tipo_usuario'] === 1);
        ...
        return redirect()->to( $isAdmin ? site_url('admin') : site_url('empleado') )
            ->with('toast',['type'=>'success','msg'=>'Bienvenido '.esc($row['nombre']).'!']);
    }
